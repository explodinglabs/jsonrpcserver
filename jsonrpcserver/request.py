...
class Request(object):
    ...
    def call_context(self):
        try:
            ...
            if self.is_notification and not config.notification_errors:
                ...
                self.response = ExceptionResponse(exc, self.request_id)
        else:
            # Return Notification or RequestResponse
            if self.is_notification:
                self.response = NotificationResponse()
            else:
                self.response = RequestResponse(self.request_id, self.result)
        # Ensure the response is set
        assert self.response, 'Context must set response'
        assert type(self.response) in (
    ...
    def call(self, methods):
        ...
        with self.call_context():
            ...
            self.result = callable_(*(self.args or []), **(self.kwargs or {}))
